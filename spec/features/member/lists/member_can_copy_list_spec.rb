...
feature 'Member can copy an existing list' do
  ...
  shared_examples_for 'copying the list details' do
    scenario 'the member can access the copy list button and clone the list' do
      within 'table' do
        within_row(list.name) { click_on 'Copy list'}
      end
  ...
  context 'the gift day has not yet occured and the list is not locked' do
    ...
    scenario 'member cannot see the clone list button' do
      within 'table' do
        within_row(list.name) do
          expect(page).to_not have_link('Copy list')
        end
      end
    end
  end
